...
class BookingAnalyticsService {
  ...
  double totalOutstanding(List<Booking> bookings) =>
      bookings.where((b) => b.isOutstanding).fold(0.0, (sum, b) => sum + b.charge);

  int freeTowCount(List<Booking> bookings) => bookings.where((b) => b.charge == 0).length;

  int paidTowCount(List<Booking> bookings) => bookings.where((b) => b.charge > 0).length;
  ...
  List<MapEntry<DateTime, int>> bookingsPerDay(List<Booking> bookings, {int days = 7, DateTime? now}) {
